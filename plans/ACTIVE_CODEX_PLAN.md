# ACTIVE CODEX PLAN

Active mission packet:

```text
plans/codex/2026-05-14_r3_structural_mechanism_grand_campaign_packet.md
```

Recent prior packet:

```text
plans/codex/2026-05-14_codex_specialized_capability_validation_packet.md
```

Prior deep-structure packet:

```text
plans/codex/2026-05-14_r3_deep_structure_spectrum_determinant_packet.md
```

Prior canonical insertion packet:

```text
plans/codex/2026-05-14_r3_canonical_insert_and_structure_packet.md
```

Prior canonical proposal patch packet:

```text
plans/codex/2026-05-13_r3_canonical_proposal_patch_packet.md
```

Prior factorization audit packet:

```text
plans/codex/2026-05-13_r3_factorization_audit_packet.md
```

Prior reconnaissance packet:

```text
plans/codex/2026-05-13_r3_next_research_cycle.md
```

Mandatory publication addendum:

```text
plans/codex/2026-05-13_r3_next_research_cycle_publish_addendum.md
```

## One-word launch

From repo root, after pulling `main`, start Codex and run:

```text
$collatz-research-orchestrator go
```

`go` means: read this active plan, read the active mission packet, read the publication addendum, execute the mission, commit safe results, and push them to GitHub.

## Expanded equivalent command

```text
$collatz-research-orchestrator go. Read plans/ACTIVE_CODEX_PLAN.md, execute the active mission packet exactly, obey the mandatory publication addendum, use subagents where useful, commit and push safe results to GitHub, and do not claim or imply a Collatz proof.
```

## Required boundary

This active plan is finite-level only. It must not be used to claim, imply, or suggest a proof of the Collatz conjecture.
